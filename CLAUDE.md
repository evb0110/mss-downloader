...
- **v1.3.25:** Fixed Orleans Médiathèques image URL extraction to use correct `/files/large/{hash}.jpg` pattern instead of problematic IIIF URLs from `o:source` field. Updated Orleans manifest loading to extract image URLs from `thumbnail_display_urls.large` field or construct URLs using `o:filename` hash. Improved Orleans API reliability with smaller batch sizes (4 items), longer delays (3s), increased failure tolerance (30%), and stall detection. Orleans manuscripts now download full-resolution images using the proper Aurelia server endpoint pattern.
- **v1.3.26:** Comprehensive fix for four manuscript libraries with critical URL and functionality issues: (1) **NYPL** - Fixed broken IIIF manifest URLs by switching from failed IIIF service to working `images.nypl.org` format with proper carousel data extraction; (2) **Modena Archive** - Fixed hardcoded 231 pages issue and added to skip size estimation list to prevent hanging during download; (3) **University of Graz** - Enhanced image resolution by switching from low-quality IIIF service URLs to high-resolution webcache URLs for full 5.3MB images instead of 271KB thumbnails; (4) **Orleans Médiathèques** - Completed transition from problematic IIIF `o:source` URLs to reliable `/files/large/{hash}.jpg` pattern for consistent full-resolution downloads. All four libraries now provide improved image quality and resolved hanging/timeout issues.
