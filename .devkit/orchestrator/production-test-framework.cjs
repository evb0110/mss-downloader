...
if (!fs.existsSync(srcPath)) {
    console.error('ERROR: Cannot find production SharedManifestLoaders.js');
    process.exit(1);
}
...
class ProductionCodeTester {
    ...
    async testLibrary(testId, config) {
        ...
        try {
            ...
            } else if (manifest.images && manifest.images[0]) {
                imageUrl = manifest.images[0].url || manifest.images[0]['@id'];
            }
            if (imageUrl) {
    ...
    extractImageUrl(canvas) {
        if (canvas.images && canvas.images[0]) {
            const image = canvas.images[0];
            if (image.resource) {
                return image.resource['@id'] || image.resource.id;
            }
        }
        return null;
    }
...
module.exports = { ProductionCodeTester, TEST_CASES };