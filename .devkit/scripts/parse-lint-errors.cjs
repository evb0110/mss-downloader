...
// Take first 100 errors
const first100 = errors.slice(0, 100);

// Divide into 4 groups
const groupSize = Math.ceil(first100.length / 4);
const groups = [];
for (let i = 0; i < 4; i++) {
  groups.push(first100.slice(i * groupSize, (i + 1) * groupSize));
}

// Create summary
console.log(`Total errors found: ${errors.length}`);
console.log(`Processing first 100 errors\n`);

// Group by error type
const errorTypes = {};
first100.forEach(err => {
  const key = err.rule;
  if (!errorTypes[key]) {
    errorTypes[key] = [];
  }
...
groups.forEach((group, index) => {
  const agentFile = path.join('.devkit', 'scripts', `agent-${index + 1}-errors.json`);
...
fs.writeFileSync('.devkit/scripts/first-100-errors.json', JSON.stringify(first100, null, 2));