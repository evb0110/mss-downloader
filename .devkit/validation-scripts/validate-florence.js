...
const VALIDATION_DIR = path.join(__dirname, '../validation-results/florence');
const PDF_DIR = path.join(VALIDATION_DIR, 'pdfs');

// Test URLs from GitHub issue #5
const TEST_URLS = [
    ...
    'https://cdm21059.contentdm.oclc.org/digital/collection/plutei/id/17607'
];
...
async function validateFlorence() {
    console.log('=== Florence Library Validation ===\n');
    ...
    for (const url of TEST_URLS) {
        ...
        try {
            ...
            console.log(`  Title: ${manifest.displayName || 'Florence Manuscript'}`);
            ...
            if (imageFiles.length > 0) {
                // Create PDF
                const pdfPath = path.join(PDF_DIR, `florence_${manuscriptId}.pdf`);
    ...
    // Write report
    const report = {
        library: 'Florence (ContentDM)',
        timestamp: new Date().toISOString(),
        issue: 'GitHub Issue #5 - ETIMEDOUT cdm21059.contentdm.oclc.org',
        results,
        summary: {
...
// Run validation
validateFlorence().catch(console.error);