...
const VALIDATION_DIR = path.join(__dirname, '../validation-results/verona');
const PDF_DIR = path.join(VALIDATION_DIR, 'pdfs');

// Test URLs from GitHub issue #3
const TEST_URLS = [
    'https://nuovabibliotecamanoscritta.it/catalogo/Scheda?codice=15',
    'https://nuovabibliotecamanoscritta.it/catalogo/Scheda?codice=14',
...
async function validateVerona() {
    console.log('=== Verona Library Validation ===\n');
    ...
    for (const url of TEST_URLS) {
        console.log(`\nTesting: ${url}`);
        const manuscriptId = url.includes('codice=') ? url.match(/codice=(\d+)/)[1] : 
                           url.match(/manifest\/([^.]+)\.json/)?.[1] || 'unknown';
        ...
        try {
            ...
            console.log(`  Title: ${manifest.displayName || 'Verona Manuscript'}`);
            ...
            if (imageFiles.length > 0) {
                // Create PDF
                const pdfPath = path.join(PDF_DIR, `verona_${manuscriptId}.pdf`);
    ...
    const report = {
        ...
        issue: 'GitHub Issue #3 - ETIMEDOUT nuovabibliotecamanoscritta.it',
        results,
        summary: {
...
// Run validation
validateVerona().catch(console.error);