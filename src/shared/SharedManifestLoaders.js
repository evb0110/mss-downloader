...
exports.SharedManifestLoaders = void 0;
class SharedManifestLoaders {
    ...
    async defaultNodeFetch(url, options = {}, retries = 3) {
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                if (error.code)
                    console.log(`[SharedManifestLoaders] Error code: ${error.code}`);
                if (i === retries - 1) {
    ...
    async getBDLManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid BDL URL');
        ...
        if (!response.ok)
            throw new Error(`Failed to fetch BDL API: ${response.status}`);
    ...
    async getViennaManuscriptaManifest(url) {
        ...
        if (!urlMatch)
            throw new Error('Invalid Vienna Manuscripta URL');
        ...
        if (!parts)
            throw new Error('Invalid manuscript ID format');
    ...
    async getBNEManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid BNE URL');
        const docId = match[1];
        const images = [];
    ...
    async getKarlsruheManifest(url) {
        ...
        if (!response.ok)
            throw new Error(`Failed to fetch Karlsruhe manifest: ${response.status} ${response.statusText}`);
    ...
    async getLibraryOfCongressManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid Library of Congress URL');
    ...
    async getGrazManifest(url) {
        ...
        try {
            ...
            const labelUtils = await Promise.resolve().then(() => __importStar(require('../shared/utils/labelUtils.js')));
            ...
            const safeError = new Error(`Failed to load University of Graz manifest: ${error.message || 'Unknown network or server error'}`);
    ...
    async getMDCCataloniaManifest(url) {
        ...
        if (!item)
            throw new Error('Could not find item data in page state');
        const images = [];
    ...
    async getBVPBManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid BVPB URL');
        const registroId = match[1];
        // Fetch the registro page to find image viewer links
        const response = await this.fetchWithRetry(url);
        if (!response.ok)
            throw new Error(`Failed to fetch BVPB page: ${response.status}`);
        const html = await response.text();
        ...
        if (!grupoResponse.ok)
            throw new Error(`Failed to fetch grupo page: ${grupoResponse.status}`);
    ...
    async getMorganManifest(url) {
        ...
        if (url.includes('ica.themorgan.org')) {
            ...
            if (!manuscriptId)
                throw new Error('Invalid Morgan URL format. Expected patterns: /manuscript/XXXXX, /manuscripts/XXXXX, or /collection/XXXXX');
        ...
        if (url.includes('ica.themorgan.org')) {
            ...
            try {
                ...
                const pagePromises = uniquePages.slice(0, maxPagesToFetch).map(async (pageNum) => {
                    ...
                    try {
                        ...
                        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Page fetch timeout')), 5000));
            ...
            if (uniquePages.length > images.length) {
                ...
                images.forEach((img) => {
                    ...
                    if (match)
                        existingPageNumbers.add(match[1]);
                });
    ...
    async getHHUManifest(url) {
        ...
        const match = url.match(/titleinfo\/(\d+)/);
        if (!match)
            throw new Error('Invalid HHU URL format');
...
If you have a UniPub URL (starting with https://unipub.uni-graz.at/), please use that instead.`);
    ...
    async getFlorenceManifest(url) {
        ...
        if (!itemId)
            throw new Error('Invalid Florence URL. Expected patterns: /item/XXXXX or /collection/plutei/id/XXXXX');
                ...
                if (manifestText.trim().startsWith('{') || manifestText.trim().startsWith('[')) {
                    ...
                    if (iiifResult.images.length === 1) {
                        ...
                        try {
                            ...
                            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Compound object detection timeout after 60s')), 60000));
    ...
    async getManchesterManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid Manchester URL');
        const manuscriptId = match[1];
    ...
    async getMunichManifest(url) {
        ...
        if (manuscriptId.startsWith('bsb000')) {
            ...
            const shelfMarkEntry = manifest.metadata.find((m) => m.label === 'Signatur' ||
                m.label === 'Shelf mark' ||
                m.label === 'Call Number');
    ...
    async getTorontoManifest(url) {
        ...
        if (url.includes('collections.library.utoronto.ca')) {
            ...
            if (!viewMatch)
                throw new Error('Invalid Toronto collections URL');
    ...
    async getVaticanManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid Vatican Library URL');
        const manuscriptId = match[1];
    ...
    async discoverBordeauxPageRange(baseId) {
        ...
        for (const page of quickScanPages) {
            ...
            try {
                ...
                if (response.ok) {
                    foundAny = true;
                    if (minFound === null || page < minFound)
                        minFound = page;
                    if (maxFound === null || page > maxFound)
                        maxFound = page;
        ...
        for (let batchStart = detailedStart; batchStart <= detailedEnd; batchStart += batchSize) {
            ...
            for (let page = batchStart; page <= batchEnd; page++) {
                const pageId = `${baseId}_${String(page).padStart(4, '0')}`;
                const testUrl = `${baseUrl}/${pageId}_files/0/0_0.jpg`;
                batchPromises.push(this.fetchUrl(testUrl)
    ...
    async getBodleianManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid Bodleian URL');
        const objectId = match[1];
    ...
    async discoverEManuscriptaBlocks(baseManuscriptId, library) {
        ...
        if (sortedAll.length > 0) {
            ...
            if (sequences.length > 0) {
                coreBlocks = sequences.reduce((longest, current) => current.length > longest.length ? current : longest, []);
                ...
                if (coreBlocks.length > 0 && technicalBlocks.length > 0) {
                    ...
                    technicalBlocks = technicalBlocks.filter(block => {
                        const distanceFromCore = Math.min(Math.abs(block - coreStart), Math.abs(block - coreEnd));
    ...
    async getNorwegianManifest(url) {
        ...
        if (!match)
            throw new Error('Invalid Norwegian National Library URL');
        const itemId = match[1];
