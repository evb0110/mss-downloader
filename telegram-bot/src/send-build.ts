...
import { MSSTelegramBot } from './legacy-bot.js';
import { BuildUtils } from './build-utils.js';
import type { FileResult } from './types.js';
import { isMainModule } from './utils.js';

console.warn('⚠️  WARNING: send-build.ts is DEPRECATED');
console.warn('📱 Use send-multiplatform-build.ts for multiplatform support');
...
function getChangelogFromCommits(version: string): string {
  try {
    ...
    const commits = execSync('git log --oneline -15 --pretty=format:"%s"', { encoding: 'utf8' })
      .trim()
      .split('\n');
  ...
  try {
    ...
    if (versionHistoryMatch) {
      ...
      const recentVersions = versionHistory
        ...
        .filter((item): item is VersionInfo => item !== null)
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error('Error reading version history:', errorMessage);
  }
  
  return `${bold("📝 What's New:")}\n• Latest updates and improvements`;
}

// BuildResult type is handled by BuildUtils.findSinglePlatformBuild return type
...
async function sendBuild(): Promise<void> {
  try {
    ...
    const buildFile = buildResult.buildFileName || path.basename(buildResult.buildFile);
...
📥 Download and install to get the latest features and fixes!
    ...
    bot.notifySubscribers = async function(message: string, file?: string) {
      // Call original with validation wrapper
      const originalSendFileToSubscriber = bot.sendFileToSubscriber.bind(bot);
      bot.sendFileToSubscriber = async function(chatId: number, message: string, fileResult: FileResult) {
...
Usage:
  bun run send-build.ts                 Send latest build to subscribers
  bun run send-build.ts --help          Show this help
  bun run send-build.ts --message "msg" Send custom message (no file)
...
Examples:
  ...
  npm run dist
  bun run telegram-bot/src/send-build.ts
  `);
}
...
async function main(): Promise<void> {
  ...
  if (messageIndex !== -1 && args[messageIndex + 1]) {
    await sendMessage(args[messageIndex + 1]);
  } else {
    await sendBuild();
  }
}
