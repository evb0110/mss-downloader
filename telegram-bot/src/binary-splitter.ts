...
export class BinarySplitter {
  ...
  async splitFile(filePath: string): Promise<SplitResult> {
  ...
  private createRecombinationScript(nameWithoutExt: string, ext: string, totalParts: number): string {
...
`;
  ...
  private getInstructions(nameWithoutExt: string, ext: string, totalParts: number): string {
