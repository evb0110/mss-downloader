...
class BotTester {
    ...
    constructor() {
        ...
        this.testChatId = parseInt(process.env.TEST_CHAT_ID || '53582187'); // Default to admin chat ID
    ...
    addResult(test: string, success: boolean, message: string, details: string | null = null): void {
    ...
    async sendCommand(command: string): Promise<TelegramBot.Message | null> {
        try {
            ...
            this.addResult(`Command ${command}`, false, `Failed to send command`, errorMessage);
            return null;
        }
    }
    
    async testStartCommand(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('/start command', false, 'Failed to send /start command', errorMessage);
        }
    }
    
    async testSubscribeCommand(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('/subscribe command', false, 'Failed to send /subscribe command', errorMessage);
        }
    }
    
    async testUnsubscribeCommand(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('/unsubscribe command', false, 'Failed to send /unsubscribe command', errorMessage);
        }
    }
    
    async testLatestCommand(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('/latest command', false, 'Failed to send /latest command', errorMessage);
        }
    }
    
    async testAdminCommand(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('/test_admin command', false, 'Failed to send /test_admin command', errorMessage);
        }
    }
    
    async testHelpCommand(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('/help command', false, 'Command not implemented or failed', errorMessage);
        }
    }
    
    async testNonCommandMessage(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('Non-command message', false, 'Failed to send non-command message', errorMessage);
        }
    }
    
    async testBotInfo(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('Bot Info', false, 'Failed to get bot information', errorMessage);
        }
    }
    ...
    async testBotCommands(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('Bot Commands', false, 'Failed to get bot commands', errorMessage);
        }
    }
    
    async testWebhookInfo(): Promise<void> {
        ...
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.addResult('Webhook Info', false, 'Failed to get webhook information', errorMessage);
        }
    }
    
    async testFileSystemChecks(): Promise<void> {
        ...
        try {
            ...
            this.addResult('Subscribers File', true, `Found ${subscribers.length} subscribers`, subscribersFile);
        } catch (error) {
            if (error && typeof error === 'object' && 'code' in error && error.code === 'ENOENT') {
                this.addResult('Subscribers File', false, 'Subscribers file not found', subscribersFile);
            } else {
                const errorMessage = error instanceof Error ? error.message : String(error);
                this.addResult('Subscribers File', false, 'Invalid JSON in subscribers file', errorMessage);
            }
        }
        ...
        try {
            ...
            const buildFiles = files.filter(f => f.endsWith('.exe') || f.endsWith('.AppImage'));
            this.addResult('Build Files', true, `Found ${buildFiles.length} build files in dist/`, buildFiles.join(', '));
        } catch (error) {
            this.addResult('Build Files', false, 'No dist directory found', distDir);
        }
    }
    
    async testModuleDependencies(): Promise<void> {
        ...
        for (const moduleName of requiredModules) {
            try {
                if (moduleName.startsWith('./')) {
                    // Check if local module file exists
                    const modulePath = path.join(process.cwd(), moduleName);
                    await fs.access(modulePath + '.js').catch(() => fs.access(modulePath + '.ts'));
                    this.addResult(`Module: ${moduleName}`, true, 'Module file found', null);
                } else {
                ...
                const errorMessage = error instanceof Error ? error.message : String(error);
                this.addResult(`Module: ${moduleName}`, false, 'Failed to load module', errorMessage);
            }
        }
    }
